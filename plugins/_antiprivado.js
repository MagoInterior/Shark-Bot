...
if (m.text.includes('PIEDRA') || m.text.includes('PAPEL') || m.text.includes('TIJERA') 
    ...
    || m.text.includes('creador') || m.text.includes('ping') || m.text.includes('bottemporal') || m.text.includes('gruposgb') 
    || m.text.includes('instalarbot') || m.text.includes('términos') || m.text.includes('donar')) return !0
let chat = global.db.data.chats[m.chat]
let bot = global.db.data.settings[this.user.jid] || {}
let user = global.db.data.users[m.sender]
if (bot.antiPrivate && !isOwner && !isROwner) {
await m.reply(`*@${m.sender.split`@`[0]} NO PUEDE USAR ESTE BOT EN CHAT PRIVADO, SERÁ BANEADO*`, false, { mentions: [m.sender] })
