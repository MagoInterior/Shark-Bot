...
let nivelPescar = pancing == 0 ? 'No tengo' : '' || pancing == 1 ? 'Nivel ✦ 1' : '' || pancing == 2 ? 'Nivel ✦ 2' : '' || pancing == 3 ? 'Nivel ✦ 3' : '' || pancing == 4 ? 'Nivel ✦ 4' : '' || pancing >= 5 ? 'Nivel ✦ 5 ǁ MAX' : ''
let nivelGancho = pancingan == 0 ? 'No tengo | I do not have' : '' || pancingan == 1 ? 'Nivel ✦ 1' : '' || pancingan == 2 ? 'Nivel ✦ 2' : '' || pancingan == 3 ? 'Nivel ✦ 3' : '' || pancingan == 4 ? 'Nivel ✦ 4' : '' || pancingan >= 5 ? 'Nivel ✦ 5 ǁ MAX' : ''
...
if (user.stamina < 59) return conn.sendButton( m.chat, `${ag}𝙉𝙊 𝙏𝙄𝙀𝙉𝙀𝙎 𝙎𝙐𝙁𝙄𝙀𝙉𝙏𝙀 𝙀𝙉𝙀??𝙂𝙄𝘼. 𝙈𝙄𝙉𝙄𝙈𝙊 𝙉𝙀𝘾𝙀𝙎𝙄𝙏𝘼𝙎 𝙐𝙉 *${energia[3]}%* 𝘿𝙀 𝙀𝙉𝙀𝙍𝙂𝙄𝘼 ✨`, `𝙀𝙉𝙀𝙍𝙂𝙄𝘼 𝘼𝘾𝙏𝙐𝘼𝙇: ${rpgg.emoticon('stamina')} *${user.stamina}%*\n` + wm, [[`𝘾𝙊𝙈𝙋𝙍𝘼𝙍 ${energia[3]}% ${rpgg.emoticon('stamina')}`, '.buy stamina 60'], [`🎒 𝙄𝙉𝙑𝙀𝙉𝙏𝘼𝙍𝙄𝙊`, `.inventario`], [`𝗠 𝗘 𝗡 𝗨 ☘️`, `.menu`]], fkontak, m)   
